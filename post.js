...
import {
  ...
  setDoc, getDocs, collection, addDoc
...
const webhookBase64 = "aHR0cHM6Ly9kaXNjb3JkLmNvbS9hcGkvd2ViaG9va3MvMTM4MzQzNjkyMzAzMjMwOTc2MC9CZWllbjEtOVg3X0szZVlGYUl0QWZSYTFwb3NSQnRDWGZQYW9GWDBnQlFfVDMzdTgtb1BIXzU1aXJfS0s5OTI5NzBwVQ==";
...
async function loadPost() {
  const docRef = doc(db, "prompts", postId);
  ...
  postContainer.innerHTML = `
    <div class="prompt-content">
      ...
      <span id="reportBtn" class="material-icons" style="position:absolute;top:10px;right:10px;">flag</span>
    </div>
  `;
...
async function loadComments() {
  comments.innerHTML = "";
  const snap = await getDocs(collection(db, "prompts", postId, "comments"));
  snap.forEach(doc => {
    const c = doc.data();
...
sendComment.onclick = async () => {
  ...
  if (!text) return showAlert("Comment is empty", "error");
...
likeBtn.onclick = async () => {
  ...
  if (localStorage.getItem(key)) return showAlert("Already liked", "error");

  await updateDoc(doc(db, "prompts", postId), {
    likes: increment(1)
  });
